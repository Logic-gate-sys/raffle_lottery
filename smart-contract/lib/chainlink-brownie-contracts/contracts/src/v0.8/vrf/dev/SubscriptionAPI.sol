...
abstract contract SubscriptionAPI is ConfirmedOwner, IERC677Receiver, IVRFSubscriptionV2Plus {
  ...
  function createSubscription() external override nonReentrant returns (uint256 subId) {
    ...
    // ------->>> { I added this because of underflow issues with block (0 -1) }
    bytes32 block_hash =block.number > 0 ? blockhash(block.number - 1) :  blockhash(0) ;
    subId = uint256(
      keccak256(abi.encodePacked(msg.sender, block_hash, address(this), currentSubNonce))
    );
